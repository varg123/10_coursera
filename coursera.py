...
def get_courses_list(count=20):
    ...
    for it in choices(BeautifulSoup(resp.text, features='lxml').find_all('loc'), k=count):
        yield it.get_text()
...
def get_course_info(course_url):
    ...
    course_data = json.loads(bs.select_one('script[type="application/ld+json"]').get_text())
    ...
    course_info = namedtuple('course_info', 'name language start_date weeks_count rating')
